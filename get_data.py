...
class TIMIT(Dataset):
    ...
    def __init__(self,dir , transform = None,quiry ='TRAIN'):
        ...
        self.labels = self.TRAIN_data_filtered['lbl'].unique()
        self.label_enum_dict = dict(zip(self.labels,np.arange(0,self.labels.size)))
    def USAGE(self):
    ...
    def __read_pnm(self,pnm_dict):
        ...
        return {'samp' : samp[pnm_dict['start']:pnm_dict['end']], 'lbl': self.label_enum_dict[pnm_dict['lbl']],'lbl_name':pnm_dict['lbl'],'rate' :rate}
